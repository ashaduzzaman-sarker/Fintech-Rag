...
class CohereReranker:
    ...
    def __init__(
        ...
        logger.info(
            ...
            extra={"model": self.model, "top_n": self.top_n}
        )
    ...
    def rerank(
        ...
        logger.info(
            ...
            extra={"query": query[:50], "top_n": top_n}
        )
    ...
    def compare_ranking(
        ...
        original_order = sorted(
            enumerate(documents),
            key=lambda x: x[1].get(original_key, 0),
            reverse=True
        )
        ...
        rerank_order = sorted(
            enumerate(documents),
            key=lambda x: x[1].get(rerank_key, 0),
            reverse=True
        )
        
        # Calculate rank changes
        rank_changes = []
        for new_rank, (orig_idx, doc) in enumerate(rerank_order):
            orig_rank = next(
                i for i, (idx, _) in enumerate(original_order)
                if idx == orig_idx
            )
