...
def query_rag(question: str, top_k: int = 5, include_confidence: bool = True) -> dict[str, Any]:
    ...
    try:
        response = requests.post(
            f"{API_V1}/query",
            json={"question": question, "top_k": top_k, "include_confidence": include_confidence},
            timeout=30,
        )
...
with st.sidebar:
    ...
    selected_sample = st.selectbox("Try a sample query:", [""] + sample_queries, index=0)
...
st.markdown(
    '<div class="sub-header">Ask questions about compliance, risk management, and regulations</div>',
    unsafe_allow_html=True,
)
            ...
            if show_confidence and "confidence" in result:
                ...
                with col2:
                    if confidence_level == "high":
                        st.markdown(
                            '<p class="confidence-high">High Confidence</p>', unsafe_allow_html=True
                        )
                    elif confidence_level == "medium":
                        st.markdown(
                            '<p class="confidence-medium">Medium Confidence</p>',
                            unsafe_allow_html=True,
                        )
                    else:
                        st.markdown(
                            '<p class="confidence-low">Low Confidence</p>', unsafe_allow_html=True
                        )
                with col3:
...
with tab3:
    ...
    if "error" in stats:
        ...
        with col1:
            st.metric("Documents Indexed", f"{stats.get('total_documents_indexed', 0):,}")
        with col2:
            st.metric("Queries Processed", f"{stats.get('total_queries_processed', 0):,}")
        with col3:
            st.metric("Avg Query Time", f"{stats.get('avg_query_time', 0):.2f}s")
        with col4:
...
st.markdown(
    ...
    <div style="text-align: center; color: #666; padding: 2rem 0;">
        ...
        <p style="font-size: 0.9rem;">
            <a href="{API_BASE_URL}/docs" target="_blank">API Documentation</a> |
